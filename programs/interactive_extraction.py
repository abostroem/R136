...
def get_background_locations(input_obj, fig1, ax1, pix_num, collapsed_img, cenwave, first_time = True):
    ...
    if first_time is True:
        if os.path.exists(input_obj.filename.replace('.fits', '_c%i_background_regions.txt' %(cenwave))):
            keep_file_flag = input('Background file already exists: Overwrite file (w) or Append to file (a)') #you should never enter this loop if mode = passive
        else:
            keep_file_flag = 'w'
        ofile = open(input_obj.filename.replace('.fits','_c%i_background_regions.txt' %(cenwave)), keep_file_flag)
    #Background regions have been defined - adding or removing some of them
    if first_time is False:
        remove_flag = 'y'
        #Get initial indx values from background file
        start_indx, end_indx  = np.genfromtxt(input_obj.filename.replace('.fits','_c%i_background_regions.txt' %(cenwave)), unpack = True)
        ...
        while remove_flag != 'n':
            ...
            remove_flag = interact_w_user(input_obj.mode, message = 'Would you like to remove any background regions? (y), n ', default = 'n')
            if remove_flag == 'n':
                break
            try:
                remove_indx = int(interact_w_user(input_obj.mode, message = 'Enter the number of the region you wish to remove: ', default = 1)) #you should never enter this loop if mode = passive
            except ValueError:
        ...
        ofile = open(input_obj.filename.replace('.fits','_c%i_background_regions.txt' %(cenwave)), 'w')
        for sindx, eindx in zip(start_indx, end_indx):
            ...
            ofile.write('%i\t %i \n' %(int(sindx),int(eindx)))
        finished_flag = interact_w_user(input_obj.mode, message = 'Finished entering points? (n), y ', default = 'y')
    ...
    while finished_flag != 'y':
        ...
        pyplot.draw()
        keep_flag = interact_w_user(input_obj.mode, message = 'Keep points? (y), n ', default = 'y')
        if keep_flag != 'n':
...
def fit_background(input_obj, fig1, ax1, background_pix, background_collapsed, pix_num):
    ...
    fig1.canvas.draw()
    change_deg_flag = interact_w_user(input_obj.mode, message = 'Select which order spline you would like to use for your fit (1, 2, 3, 4, 5), -or- redefine background, r ', default = 3)
    if change_deg_flag != 'r':
        while int(change_deg_flag) not in [1, 2, 3, 4, 5]:
            change_deg_flag = input('Enter a valid spline degree: ')  #you should never enter this loop if mode = passive
...
def write_temp_subtracted_file(input_obj, subtracted_img):
    '''Write the subtracted image to a temporary file for calstis to extract from. This file is erased at the end of execution '''
    shutil.copyfile(input_obj.filename, input_obj.filename.replace('.fits', 'sub.fits'))
    ofile = pyfits.open(input_obj.filename.replace('.fits', 'sub.fits'), mode = 'update')
...
def select_extraction_location(fig1, c, region_to_mark = 'spectrum'):  #you should never enter this function if mode = passive
    ...
    interact_w_user(input_obj.mode, message = 'Zoom in on spectrum, hit Enter when finished')
    print('Select %s location' %(region_to_mark))
    keep_flag = 'n'
    while keep_flag == 'n':
        coords = fig1.ginput(n = 1, timeout = -1)
        ln = pyplot.plot([coords[0][0]], [coords[0][1]], '%s+' %(c))
        keep_flag = input('Keep selection? (y), n ') 
        if keep_flag == 'n':
...
def select_extraction_box_size(fig1, extrlocy, c, region_to_mark = 'spectrum'): #you should never enter this function if mode = passive
    ...
    while keep_flag == 'n':
        ...
        pyplot.draw()
        keep_flag = input('Keep points? (y), n ')
        if keep_flag == 'n':
            lb[0].set_visible(False)
            pyplot.draw()
    return half_box*2

def extract_spectrum(input_obj, extrlocy, extract_box_size, background_loc1, background_size1, background_loc2, background_size2, c, backcorr_option, bksmode_option):
    '''Extract the spectrum using the calstis task x1d '''
    if os.path.exists(os.path.join(os.getcwd(), input_obj.filename.replace('.fits', '_loc%i.fits' %(int(extrlocy))))):
        os.remove(os.path.join(os.getcwd(), input_obj.filename.replace('.fits', '_loc%i.fits' %(int(extrlocy)))))
        
    #iraf.stsdas.hst_calib.stis.x1d(input_obj.filename.replace('.fits', 'sub.fits'), output = input_obj.filename.replace('.fits', '_loc%i.fits' %(int(extrlocy))), \
    ...
    x1d(input_obj.filename.replace('.fits', 'sub.fits'), output = input_obj.filename.replace('.fits', '_loc%i.fits' %(int(extrlocy))), \
                                      a2center = extrlocy + 1, extrsize = extract_box_size, maxsrch = 0, bk1offst = background_loc1 - extrlocy, \
    ...
    add_background_into_x1d(input_obj.filename, fit, extrlocy)
...
def add_background_into_x1d(filename, fit, extrlocy):
    ...
    ofile[1].data['gross'][:] = ofile[1].data['gross'][:] + fit[int(np.round(ofile[1].data['a2center'])) - 1]/exptime #a2center is 1 indexed, units are cts/s in the x1d, cts in the flt
    ofile[1].data['background'][:] = ofile[1].data['background'][:] + fit[int(np.round(ofile[1].data['a2center'])) - 1]/exptime #a2center is 1 indexed
    ofile.flush()
    ofile.close()
...
if __name__ == "__main__":
    ...
    if os.path.exists('/grp/hst/cdbs/oref'):
        ...
        os.environ['oref'] = '/Users/bostroem/Desktop/research/R136/test_data/'
        os.environ['myref'] = '/Users/bostroem/Desktop/research/R136/test_data/'
    ...
    input_obj = input_object(filename, options.mode)
    ...
    if os.path.exists(os.path.join(os.getcwd(), filename.replace('.fits','_c%i_background_regions.txt'%(cenwave)))):
        find_background_flag = interact_w_user(input_obj.mode, default = 'n', message = 'Select the background regions? y, (n) ')
    else:
    ...
    if find_background_flag == 'y':  #interactvely define background
        #Display 2D image
        disp_2d = interact_w_user(input_obj.mode, default = 'n', message = 'Would you like to display the 2D image? y, (n) ')
        if disp_2d == 'y':
            fig2 = pyplot.figure(2)
            ax2 = fig2.add_subplot(1,1,1)
            ax2.imshow(np.rot90(np.log10(img)), interpolation = 'nearest', origin = 'lower')
        interact_w_user(input_obj.mode, message = 'Zoom in on desired spectrum location. Press Enter when finished')
        
        #Define background regions in cross dispersion profile
        fig1, ax1, background_pix, background_collapsed = get_background_locations(input_obj, fig1, ax1, pix_num, collapsed_img, cenwave)
    ...
    ax1_temp, fit = fit_background(input_obj, fig1, ax1, background_pix, background_collapsed, pix_num)
    while ax1_temp is None:
        fig1, ax1, background_pix, background_collapsed = get_background_locations(input_obj, fig1, ax1, pix_num, collapsed_img, cenwave, first_time = False)
        ax1_temp, fit = fit_background(input_obj, fig1, ax1, background_pix, background_collapsed, pix_num)
    ax1 = ax1_temp
    ...
    write_temp_subtracted_file(input_obj, subtracted_img)
    ...
    while another_spectrum_flag != 'n':  #for each spectrum
        ...
        extract_spectrum(input_obj, extrlocy, extract_box_size, background_loc1, background_size1, background_loc2, background_size2, c, options.backcorr, options.bksmode)
        confirm_flag = interact_w_user(input_obj.mode, message = 'Would you like to confirm the location of your extraction on a 2D image? (y), n ', default = 'n')
        if confirm_flag != 'n':
            confirm_extraction_location(input_obj.filename, img, extrlocy)
        final_spec_flag = interact_w_user(input_obj.mode, message = 'Would you like to plot the final spectrum? (y), n ', default = 'n')
        if final_spec_flag != 'n':
            plot_final_spectrum(filename, extrlocy)
        another_spectrum_flag = interact_w_user(input_obj.mode, message = 'Extract another spectrum? ', default = 'n')
        i = i + 1
    os.remove(input_obj.filename.replace('.fits', 'sub.fits'))
